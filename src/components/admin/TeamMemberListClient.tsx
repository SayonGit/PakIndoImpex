...
import { ArrowUp, ArrowDown, Pencil, Trash2, Loader2, User } from "lucide-react";
import type { TeamMember } from "@prisma/client";

export function TeamMemberListClient({ initialItems }: { initialItems: TeamMember[] }) {
  const [items, setItems] = useState(initialItems);
  const [busyId, setBusyId] = useState<string | null>(null);

  async function move(id: string, direction: "up" | "down") {
    setBusyId(id);
    await fetch(`/api/admin/team/${id}/move`, {
      method: "POST",
    ...
    <div className="shadow-soft divide-y divide-ink-100 overflow-hidden rounded-2xl border border-ink-100 bg-white">
      {items.map((item, index) => (
        <div key={item.id} className="flex items-center gap-4 p-4">
          ...
          <div className="flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-primary-50 text-primary-700">
            {item.photoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element -- small fixed-size admin thumbnail, not worth next/image here
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-sm text-ink-500">{item.role}</p>
          </div>
