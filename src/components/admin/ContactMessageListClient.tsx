...
export function ContactMessageListClient({ initialItems }: { initialItems: ContactMessage[] }) {
        ...
        <div key={item.id} className="flex items-center gap-4 p-4">
          <div className="min-w-0 flex-1">
            <p className="truncate font-semibold text-ink-950">
              {item.name} {item.subject && <span className="font-normal text-ink-500">— {item.subject}</span>}
            </p>
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <Link
              href={`/admin/submissions/contact/${item.id}`}
