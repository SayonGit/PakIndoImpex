...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (neighbor) {
    await prisma.$transaction([
      prisma.teamMember.update({ where: { id: current.id }, data: { sortOrder: neighbor.sortOrder } }),
      prisma.teamMember.update({ where: { id: neighbor.id }, data: { sortOrder: current.sortOrder } }),
    ]);
  }
